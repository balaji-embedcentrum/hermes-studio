...
export default function FilePreviewDialog({
  ...
  const loadFile = useCallback(async () => {
    ...
    try {
      if (localAgentUrl && activeWorkspacePath) {
        const result = await localReadFile(localAgentUrl, activeWorkspacePath, path)
  ...
  }, [path, localAgentUrl, activeWorkspacePath])
  ...
  const handleSave = useCallback(async () => {
    if (!path) return
    if (localAgentUrl && activeWorkspacePath) {
      await localWriteFile(localAgentUrl, activeWorkspacePath, path, content)
    } else {
    ...
    setDirty(false)
    onSaved()
  }, [content, onSaved, path, localAgentUrl, activeWorkspacePath])
